...
DWORD dwDataLength = NULL;
PBYTE pSaveBuffer = 0;
TCHAR szAppName[] = TEXT("Record1");

__declspec(dllexport) HINSTANCE hinst;
__declspec(dllexport) INT_PTR CALLBACK DlgProc(HWND, UINT, WPARAM, LPARAM);

__declspec(dllexport) void CALLBACK runner()
{
	DialogBox(hinst, TEXT("Record"), NULL, DlgProc);
}

__declspec(dllexport) BYTE* getBuffer() {
	return pSaveBuffer;
}
...
__declspec(dllexport) INT_PTR CALLBACK DlgProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
